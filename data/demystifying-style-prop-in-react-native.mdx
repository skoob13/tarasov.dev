...
Personally, I noticed the significant difference only when an app rendered more than 5000 views on the screen rendering at the same time, and it isn't a common case definitely. I wouldn't suggest to spend time optimizing the `style` prop unless you have a real use case.
